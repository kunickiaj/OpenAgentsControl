...
function getSourceTextFromPositions(location: UnknownRecord, sourceCode?: string): string | undefined {
  ...
  const endIndex = typeof end?.line === "number" && typeof end.column === "number"
    ? getPositionIndex(sourceCode, end.line, end.column)
    : sourceCode.indexOf("\n", startIndex)
  return sourceCode.slice(startIndex, endIndex === -1 || endIndex === undefined ? sourceCode.length : endIndex)
}

function positionFromByteOffset(sourceCode: string, offset: number): { line: number; column: number } {
...
function getPosition(location: UnknownRecord): { line?: number; column?: number; offset?: number } {
  ...
  if (typeof start?.line !== "number" && offset !== undefined && typeof location.sourceCode === "string") {
...
export function parseMeasuredValue(category: string, text: string): number | undefined {
  ...
  if (category.endsWith("noExcessiveLinesPerFunction")) {
    const match = text.match(/(?:has|contains)\s+(\d+)\s+lines?|lines?\s*\((\d+)\)|(\d+)\s+lines?/i)
...
export function parseBiomeDiagnostics(output: string, sourceCode?: string): LintDiagnostic[] {
  ...
  return parsed.diagnostics.flatMap((raw): LintDiagnostic[] => {
    if (!isRecord(raw) || typeof raw.category !== "string" || !raw.category.startsWith("lint/")) return []
    const location = isRecord(raw.location) ? raw.location : {}
    const description = typeof raw.description === "string" ? raw.description : getText(raw.message)
    const position = getPosition(location)
    const diagnosticPath = getDiagnosticPath(location)
    const measuredValue = parseMeasuredValue(raw.category, getText([description, raw.message]))
...
function measuredCandidateIndexes(previous: LintDiagnostic, diagnostics: LintDiagnostic[]): number[] {
  const allIndexes = diagnostics.map((_, index) => index)
  if (!previous.sourceText) return allIndexes
  const sameSource = diagnostics.flatMap((diagnostic, index) => diagnostic.sourceText === previous.sourceText ? [index] : [])
...
function compareMeasured(before: LintDiagnostic[], after: LintDiagnostic[]): LintDiagnostic[] {
  ...
  for (const previous of before) {
    ...
    for (const index of candidateIndexes.slice(1)) {
      if (diagnosticDistance(previous, unmatched[index]) < diagnosticDistance(previous, unmatched[nearestIndex])) {
        nearestIndex = index
      }
    }
    const [current] = unmatched.splice(nearestIndex, 1)
    if ((current.measuredValue ?? 0) > (previous.measuredValue ?? 0)) regressions.push(current)
  }
...
function diagnosticDistance(before: LintDiagnostic, after: LintDiagnostic): number {
  ...
  const columnDistance = before.column === undefined || after.column === undefined
    ? 0
...
function compareCountOnly(before: LintDiagnostic[], after: LintDiagnostic[]): LintDiagnostic[] {
  ...
  for (const previous of before) {
    const sameDescription = unmatched
      ...
      .filter(({ diagnostic }) => !previous.sourceText || !diagnostic.sourceText || diagnostic.sourceText === previous.sourceText)
...
export function parseApplyPatchPaths(patchText: string): ApplyPatchPath[] {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!match) continue
    const moveMatch = match[1] === "Update" ? lines[index + 1]?.match(/^\*\*\* Move to: (.+)$/) : undefined
    paths.push({
      operation: match[1] as ApplyPatchPath["operation"],
      path: match[2].trim(),
      ...(moveMatch ? { moveTo: moveMatch[1].trim() } : {}),
    })
  }
  return paths
}
...
function getApplyPatchTouchedFiles(args: UnknownRecord, worktree: string): TouchedFile[] {
  const patchText = [args.patchText, args.patch, args.text].find(value => typeof value === "string")
...
export function createLintFeedbackHooks(dependencies: HookDependencies) {
  ...
  return {
    "tool.execute.before": async (input: HookInput, output: BeforeOutput): Promise<void> => {
      ...
      let failed = false
      await Promise.all(files.map(async ({ beforePath, afterPath }) => {
        try {
          if (!beforePath) {
          ...
          diagnosticsByPath.set(afterPath, exists ? await dependencies.runDiagnostics(beforePath) : [])
        } catch {
          failed = true
        }
      }))
    ...
    "tool.execute.after": async (input: HookInput, output: AfterOutput): Promise<void> => {
      ...
      await Promise.all(Array.from(snapshot.diagnosticsByPath, async ([relativePath, before]) => {
        try {
...
async function runCommand(
  ...
  const child = spawn(command[0], [...command.slice(1), "--", relativePath], {
    cwd: worktree,
...
export const LintFeedbackPlugin: Plugin = async ({ worktree }, options?: PluginOptions) => {
  ...
  const timeoutMs = typeof settings.timeoutMs === "number" && Number.isFinite(settings.timeoutMs) && settings.timeoutMs > 0
    ? settings.timeoutMs
    : 10_000
